...
public class CheckedStage extends Activity {
    ...
    public static void onClearStage(Context context, int stage, int rating) {  //스테이지 완료시 저장
        ...
        if (gameMode.equals("classic"))
            editor.putString("stage_classic", stageTextBuf.toString());
        else if (gameMode.equals("blind"))
            editor.putString("stage_blind", stageTextBuf.toString());
        editor.commit();
    ...
    public static int onCheckStage(Context context) { //현재까지 완료된 스테이지 확인
        ...
        String stage = "0";
        if (gameMode.equals("classic"))
            stage = mode.getString("stage_classic", "0");
        else if (gameMode.equals("blind"))
            stage = mode.getString("stage_blind", "0");
