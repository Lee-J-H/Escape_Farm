    ...
    public void move(){
        ...
        for(int i=2 ; i< totalObj.length ; i++) {
            ...
            if (totalObj[i].getLength(curPosX, curPosY) < totalObj[moveIndex].getLength(curPosX, curPosY)) {
                switch (direction) {
                    case "left":
                        if (curPosY == totalObj[i].getPosY()  && curPosX > totalObj[i].getPosX()) moveIndex = i; //동일한 y축에 x값이 선택된 동물보다 작은 경우
                        break;
                    case "right":
                        if (curPosY == totalObj[i].getPosY()  && curPosX < totalObj[i].getPosX()) moveIndex = i;
                        break;
                    case "up":
                        if (curPosX == totalObj[i].getPosX() && curPosY > totalObj[i].getPosY()) moveIndex = i;
                        break;
                    case "down":
                        if (curPosX == totalObj[i].getPosX() && curPosY < totalObj[i].getPosY()) moveIndex = i;
                        break;
